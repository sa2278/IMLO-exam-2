...
def get_data_loaders(batch_size=64):
    ...
    with FileLock(os.path.expanduser("~/data.lock")):
        train_loader = torch.utils.data.DataLoader(
            datasets.Flowers102(root='~/data', split= "train", download=True, transform=trainTransform),
            batch_size=batch_size,
            shuffle=True,
        )
        test_loader = torch.utils.data.DataLoader(
            datasets.Flowers102(root='~/data', split="test", download=True, transform=testTransform),
            batch_size=batch_size,
            shuffle=True,
        )
...
def train_ResNet(config):
    ...
    model = ResNet(3, ResBottleneckBlock, [3, 4, 6, 3], useBottleneck=True, outputs=102).to(device)
